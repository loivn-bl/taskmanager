...
class TaskManager:
    def __init__(self, root):
        self.root = root
        self.root.title("Task Manager (0.1.2)")
    ...
    def setup_settings_tab(self):
        ...
        ttk.Label(info_frame, text="Version: 0.1.2 (build 3)").pack(anchor=tk.W)
...
"""
        ...
        # Deadline
        overdue = sum(1 for t in self.tasks if not t["completed"] and t["deadline"] and datetime.strptime(t["deadline"], "%H:%M %d/%m/%Y") < datetime.now())
        upcoming = sum(1 for t in self.tasks if not t["completed"] and t["deadline"] and 0 < (datetime.strptime(t["deadline"], "%H:%M %d/%m/%Y") - datetime.now()).total_seconds() <= 86400*3)
        
        stats += f"""
⏰ THEO DEADLINE:
   • Quá hạn: {overdue}
   • Sắp đến hạn (3 ngày): {upcoming}
...
"""
    ...
    def export_pdf(self):
        if not REPORTLAB_AVAILABLE:
            messagebox.showerror("Lỗi", "Cần chạy \"pip install reportlab\"")
            return
    ...
    def toggle_complete(self, task_id):
        for t in self.tasks:
            if t["id"] == task_id:
                ...
                t["progress"] = 100 if t["completed"] else 0
    ...
    def edit_task(self, task_id):
        ...
        ttk.Button(frame, text="❌ Hủy", command=win.destroy).grid(row=8, column=0, columnspan=2)
