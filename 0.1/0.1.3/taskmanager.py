...
Chúc bạn quản lý công việc hiệu quả!"""
    ...
    def refresh_task_list(self):
        ...
        for task in filtered:
            ...
            if is_overdue and not task["completed"]:
                overdue_color = "#C63434" if self.dark_mode else "#d25959"
...
"""
        ...
        for t in self.tasks:
            if not t["completed"] and t["deadline"]:
                try:
                    dl = datetime.strptime(t["deadline"], "%H:%M %d/%m/%Y")
                    if dl < datetime.now():
                        overdue += 1
                    elif (dl - datetime.now()).total_seconds() <= 86400*3:
                        upcoming += 1
                except:
                    pass
