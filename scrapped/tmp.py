...
class TaskManager:
    ...
    def setup_main_tab(self):
        ...
        filter_combo = ttk.Combobox(control_frame, textvariable=self.filter_var,
                                   values=["Tất cả", "Đang thực hiện", "Hoàn thành", "Quá hạn", "Sắp đến hạn", "Theo danh mục"],
        ...
        # Thêm cột "Trạng thái" vào đầu bảng
        columns = ("status_icon", "name", "category", "priority", "deadline", "task_status", "tags", "progress", "actions")
        self.tree = ttk.Treeview(list_frame, columns=columns, show="headings", height=15)
        self.tree.heading("status_icon", text="Đã xong")
        self.tree.heading("task_status", text="Trạng thái")
    ...
    def setup_settings_tab(self):
        ...
        if not PLYER_AVAILABLE:
            ttk.Label(notif_frame, text="⚠️ Thư viện plyer chưa được cài đặt, thông báo sẽ không hoạt động", foreground="orange").pack(anchor=tk.W, pady=5)
...
Chúc bạn quản lý công việc hiệu quả!"""
    ...
    def get_task_status_text(self, task):
        """Trả về (icon, text_status, is_overdue, is_upcoming)"""
        if task["completed"]:
            return "✅", "Hoàn thành", False, False
        if not task["deadline"]:
            return "🟢", "Bình thường", False, False
        try:
            deadline = datetime.strptime(task["deadline"], "%H:%M %d/%m/%Y")
            now = datetime.now()
            if deadline < now:
                return "🔴", "Quá hạn", True, False
            elif (deadline - now).total_seconds() <= 86400 * 2:  # 2 ngày
                return "🟡", "Sắp đến hạn", False, True
            else:
                return "🟢", "Bình thường", False, False
        except:
            return "🟢", "Bình thường", False, False
    ...
    def refresh_task_list(self):
        ...
        for task in filtered:
            icon, status_text, is_overdue, is_upcoming = self.get_task_status_text(task)
            ...
            if is_overdue:
                ...
                self.tree.tag_configure(f"overdue_{task['id']}", background=overdue_color)
                self.tree.item(item_id, tags=(f"overdue_{task['id']}",))
            elif is_upcoming:
                upcoming_color = "#8b6b3a" if self.dark_mode else "#FFF4CC"
                self.tree.tag_configure(f"upcoming_{task['id']}", background=upcoming_color)
                self.tree.item(item_id, tags=(f"upcoming_{task['id']}",))
    ...
    def filter_tasks(self):
        ...
        if filter_type == "Đang thực hiện":
            ...
            return [t for t in self.tasks if not t["completed"] and self.is_overdue(t)]
        elif filter_type == "Sắp đến hạn":
            return [t for t in self.tasks if not t["completed"] and self.is_upcoming(t)]
    ...
    def is_upcoming(self, task):
        if task["completed"] or not task["deadline"]:
            return False
        try:
            deadline = datetime.strptime(task["deadline"], "%H:%M %d/%m/%Y")
            now = datetime.now()
            return 0 < (deadline - now).total_seconds() <= 86400 * 2
        except:
            return False
    ...
    def sort_tasks(self, tasks):
        ...
        if key == "priority":
            ...
            # Sắp xếp theo trạng thái: ưu tiên quá hạn, sắp đến hạn, bình thường, hoàn thành
            def status_order(t):
                if t["completed"]:
                    return 4
                if self.is_overdue(t):
                    return 1
                if self.is_upcoming(t):
                    return 2
                return 3
            return sorted(tasks, key=status_order, reverse=reverse)
        else:
            return tasks
...
"""
        ...
        for t in recent:
            ...
            stats += f"   • {status} {t['name']} - {t.get('created_at','N/A')}\n"
    ...
    def on_tree_click(self, event):
        ...
        if region == "cell":
            ...
            if item:
                ...
                if values:
                    task_name = values[2]  # cột name ở vị trí index 2
                    ...
                    if task:
                        if col == "#8":
                            ...
                            menu.add_command(label="📊 Cập nhật tiến độ",
                                            command=lambda: self.update_progress_dialog(task["id"]))
                            menu.add_separator()
    ...
    def update_progress_dialog(self, task_id):
        ...
        def update_label(*args):
            ...
            # Tự động tick completed nếu 100%, ngược lại bỏ tick
    ...
    def check_deadlines_and_notify(self):
        """Kiểm tra deadline và gửi thông báo (chạy trong thread)"""
        if not self.notification_var.get():
            return
        if not PLYER_AVAILABLE:
            return
        while True:
                    ...
                    try:
                        ...
                        if 0 < diff.total_seconds() <= 3600:
                            notification.notify(
                                ...
                                message=f"Công việc '{task['name']}' sẽ hết hạn trong 1 giờ tới!",
                                timeout=10
                            )
                        elif 0 < diff.total_seconds() <= 86400:
                            notification.notify(
                                title="⚠️ Nhắc nhở deadline",
                                message=f"Công việc '{task['name']}' sẽ hết hạn trong 1 ngày nữa!",
                                timeout=10
                            )
                        elif diff.total_seconds() < 0:
                            notification.notify(
                                title="❗ Công việc đã quá hạn!",
                                message=f"Công việc '{task['name']}' đã quá deadline!",
                                timeout=10
                            )
                    except:
                        pass
    ...
    def start_background_tasks(self):
        ...
        # Khởi động thread thông báo deadline
        threading.Thread(target=self.check_deadlines_and_notify, daemon=True).start()
